    ...
    fn handle_tx(
        ...
        loop {
            ...
            if let Some((next_i, next_step)) = trace.peek() {
                ...
                // At the last step of Tx eval threshold with rws generated by EndTx & EndChunk | Endblock
                // Otherwise eval with rws generated EndChunk | Endblock
                let exceed_threshold = if *next_i == geth_trace.struct_logs.len() - 1 {
                    Self::calc_rws_with_end_tx(param.max_rws, &tx_ctx) >= param.max_rws
                } else {
                    param.max_rws >= self.circuits_params.max_rws()
                };

                // Generate EndChunk and proceed to the next if it's not the last chunk
                if is_chunked && exceed_threshold {
...
impl<C: CircuitsParams> CircuitInputBuilder<C> {
    ...
    fn calc_rws_with_end_tx(total_rws: usize, tx_ctx: &TransactionContext) -> usize {
        let end_tx_rws = 8 
            // Not the first tx in batch, query pre tx cumulative gas
            + if tx_ctx.id() > 1 { 1 } else { 0 } 
            // Not the last tx, write tx id
            + if !tx_ctx.is_last_tx() { 1 } else { 0 };

        total_rws + end_tx_rws
    }
