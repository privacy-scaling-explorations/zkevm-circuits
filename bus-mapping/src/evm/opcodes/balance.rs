...
// mod balance_tests {
//     use super::*;
//     use crate::circuit_input_builder::{ExecState, Rw};
...
//         // Check if account address is in access list as a result of bus
// mapping.         assert!(builder.sdb.add_account_to_access_list(address));
...
// ExecState::Op(OpcodeId::BALANCE))             .last()
//             .unwrap()
...
//         let operation =
// &container.tx_access_list_account[indices[4].as_usize()];         assert_eq!
// (operation.rw(), RW::WRITE);         assert_eq!(
//             operation.op(),
...
//         let operation = &container.stack[indices[6 + if exists { 1 } else { 0
// }].as_usize()];         assert_eq!(operation.rw(), RW::WRITE);
//         assert_eq!(
//             operation.op(),
//             &StackOp {
//                 call_id,
