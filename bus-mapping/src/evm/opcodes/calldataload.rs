use crate::{
    circuit_input_builder::{CallContextFieldTag, CircuitInputStateRef, ExecStep},
    Error,
};
...
// ExecState::Op(OpcodeId::CALLDATALOAD))             .unwrap();

//         let call_id =
// builder.block.txs()[0].calls()[step.call_index].call_id;         let
// caller_id = builder.block.txs()[0].calls()[step.call_index].caller_id;

//         // 1 stack read, 3 call context reads, 32 memory reads and 1 stack
// write.         assert_eq!(step.bus_mapping_instance.len(), 37);
...
//                 (
//                     RW::READ,
//                     &StackOp::new(call_id, StackAddress::from(1023),
// Word::from(offset)),                 ),
//                 (
//                     RW::WRITE,
//                     &StackOp::new(call_id, StackAddress::from(1023),
...
//         test_root_ok(0u64, calldata.clone(),
// Word::from_big_endian(&calldata));
...
//         test_internal_ok(0x20, 0x10, 0x00, pushdata,
// Word::from_big_endian(&expected));     }
// }
