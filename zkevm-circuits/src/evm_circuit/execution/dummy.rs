    ...
    for DummyGadget<F, N_POP, N_PUSH, S>
{
    ...
    fn assign_exec_step(
        ...
        if step.step.rw_indices.len() != N_POP + N_PUSH {
            log::warn!("DummyGadget: wrong number of rw indices for {:?}", step);
        }

        for i in 0..N_POP {
            let value = block.rws[step.step.rw_indices[i]].stack_value();
            self.pops[i].assign(region, offset, Some(value.to_le_bytes()))?;
        }
        for i in 0..N_PUSH {
            let value = block.rws[step.step.rw_indices[N_POP + i]].stack_value();
