...
impl<F: Field> ExecutionGadget<F> for ReturnRevertGadget<F> {
    ...
    fn assign_exec_step(
        ...
        self.opcode.assign(
            region,
            offset,
            Value::known(F::from(step.step.opcode.unwrap().as_u64())),
        )?;

        let [memory_offset, length] =
            [0, 1].map(|i| block.rws[step.step.rw_indices[i]].stack_value());
        ...
        if call.is_create && call.is_success {
            let values: Vec<_> = (3..3 + length.as_usize())
                .map(|i| block.rws[step.step.rw_indices[i]].memory_value())
                .collect();
