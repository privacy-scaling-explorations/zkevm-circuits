...
impl<F: Field> ExecutionGadget<F> for ExtcodesizeGadget<F> {
    ...
    fn assign_exec_step(
        ...
        let address = block.rws[step.step.rw_indices[0]].stack_value();
        self.address_word
        ...
        let (_, is_warm) = block.rws[step.step.rw_indices[4]].tx_access_list_value_pair();
        self.is_warm
            .assign(region, offset, Value::known(F::from(is_warm)))?;

        let code_hash = block.rws[step.step.rw_indices[5]].account_value_pair().0;
        self.code_hash
            .assign(region, offset, region.word_rlc(code_hash))?;
        self.not_exists
            .assign_value(region, offset, region.word_rlc(code_hash))?;

        let code_size = block.rws[step.step.rw_indices[6]].stack_value().as_u64();
        self.code_size
