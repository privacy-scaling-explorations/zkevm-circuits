...
impl<F: Field> ExecutionGadget<F> for SstoreGadget<F> {
    ...
    fn assign_exec_step(
        ...
        let [key, value] = [step.step.rw_indices[5], step.step.rw_indices[6]]
        ...
        let (_, value_prev, _, original_value) =
            block.rws[step.step.rw_indices[7]].storage_value_aux();
        ...
        let (_, is_warm) = block.rws[step.step.rw_indices[8]].tx_access_list_value_pair();
        self.is_warm
            .assign(region, offset, Value::known(F::from(is_warm as u64)))?;

        let (tx_refund, tx_refund_prev) = block.rws[step.step.rw_indices[9]].tx_refund_value_pair();
        self.tx_refund_prev
