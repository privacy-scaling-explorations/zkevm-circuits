...
impl<F: FieldExt> LtGadget<F> {
    fn assign_success(
        ...
        self.success.swap.assign(
            region,
            offset,
            Some(F::from((execution_step.opcode == OpcodeId::GT) as u64)),
        )?;
        ...
        self.success
            ...
            .map(|(alloc, value)| {
                alloc.assign(region, offset, Some(F::from(*value as u64)))
            })
            .collect::<Result<Vec<_>, _>>()?;
        self.success.carry.assign(
            region,
            offset,
            Some(F::from(execution_step.values[4].to_bytes_le()[0] as u64)),
        )?;
        let sumc_digits = execution_step.values[5].to_u64_digits();
        let sumc = F::from(if sumc_digits.is_empty() {
            0u64
        } else {
            sumc_digits[0]
        });
...
mod test {
    ...
    macro_rules! try_test_circuit {
        ($execution_step:expr, $operations:expr, $result:expr) => {{
            let circuit = TestCircuit::<Fp>::new($execution_step, $operations);
