...
// Max degree allowed in expressions generated by the ConstraintBuilder
// (all expressions passed into the builder are not checked).
const DEFAULT_MAX_GEN_DEGREE: usize = 4;
...
impl<F: FieldExt> ConstraintBuilder<F> {
    ...
    fn init(call_id: Option<Expression<F>>) -> Self {
        ConstraintBuilder {
            ...
            stack_offset: 0,
            call_id,
            max_degree: DEFAULT_MAX_GEN_DEGREE,
        }
    }
    ...
    pub(crate) fn require_in_set(
        ...
        assert!(set.len() <= self.max_degree, "Set too large: {}", set.len());
