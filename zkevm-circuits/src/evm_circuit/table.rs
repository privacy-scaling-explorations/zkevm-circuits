...
pub struct RwValues<F> {
    ...
    pub value: WordLimbs<Expression<F>>,
    pub value_prev: WordLimbs<Expression<F>>,
...
impl<F: Field> RwValues<F> {
    ...
    pub fn new(
        ...
        value: WordLimbs<Expression<F>>,
        value_prev: WordLimbs<Expression<F>>,
        aux1: Expression<F>,
        aux2: Expression<F>,
    ) -> Self {
        Self {
    ...
    pub(crate) fn input_exprs(&self) -> Vec<Expression<F>> {
        match self {
            ...
            Self::Rw {
                ...
                values.value.to_vec().clone(),
                values.value_prev.to_vec().clone(),
