...
#[derive(Clone, Debug)]
pub(crate) struct ShlWordsGadget<F> {
    a: util::Word<F>,
    shift: util::Word<F>,
    b: util::Word<F>,
    ...
    b64s: [Cell<F>; 4],
    // lower `64 - shf_mod64` bits
    a64s_lo: [Cell<F>; 4],
    // higher `shf_mod64` bits
    a64s_hi: [Cell<F>; 4],
    // shift[0] / 64
    shf_div64: Cell<F>,
    // shift[0] % 64
...
impl<F: Field> ShlWordsGadget<F> {
    pub(crate) fn construct(
        ...
        cb.require_equal(
            "b64s[0] == shf_div64_eq0 * a64s_lo[0] * p_hi",
            b64s[0].expr(),
            shf_div64_eq0.expr() * a64s_lo[0].expr() * p_hi.expr(),
        );
        cb.require_equal(
            "b64s[1] == \
            ...
            b64s[1].expr(),
            shf_div64_eq0.expr() * (a64s_hi[0].expr() + a64s_lo[1].expr() * p_hi.expr())
                + shf_div64_eq1.expr() * a64s_lo[0].expr() * p_hi.expr(),
        );
        cb.require_equal(
            "b64s[2] == \
            ...
            b64s[2].expr(),
            shf_div64_eq0.expr() * (a64s_hi[1].expr() + a64s_lo[2].expr() * p_hi.expr())
                + shf_div64_eq1.expr() * (a64s_hi[0].expr() + a64s_lo[1].expr() * p_hi.expr())
                + shf_div64_eq2.expr() * a64s_lo[0].expr() * p_hi.expr(),
        );
        cb.require_equal(
            "b64s[3] == \
            ...
            b64s[3].expr(),
            shf_div64_eq0.expr() * (a64s_hi[2].expr() + a64s_lo[3].expr() * p_hi.expr())
                + shf_div64_eq1.expr() * (a64s_hi[1].expr() + a64s_lo[2].expr() * p_hi.expr())
                + shf_div64_eq2.expr() * (a64s_hi[0].expr() + a64s_lo[1].expr() * p_hi.expr())
                + (1.expr() - shf_div64_eq0.expr() - shf_div64_eq1.expr() - shf_div64_eq2.expr())
                    * a64s_lo[0].expr()
                    * p_hi.expr(),
        );
        ...
        // p_lo == pow(2, 64 - shf_mod64)
        cb.add_lookup(
            "Pow2 lookup",
            Lookup::Fixed {
                tag: FixedTableTag::Pow2.expr(),
                values: [64.expr() - shf_mod64.expr(), p_lo.expr(), 0.expr()],
            },
        );

        // p_hi == pow(2, shf_mod64)
        cb.add_lookup(
            "Pow2 lookup",
            Lookup::Fixed {
                tag: FixedTableTag::Pow2.expr(),
                values: [shf_mod64.expr(), p_hi.expr(), 0.expr()],
            },
        );
    ...
    fn assign_witness(
        ...
        let p_lo: u128 = 1 << (64 - shf_mod64);
        let p_hi: u128 = 1 << shf_mod64;
        let shf_lt256 = shift
            .to_le_bytes()
            .iter()
        ...
        b64s[shf_div64 as usize] = a64s_lo[0] * p_hi;
        for k in 1..4 - shf_div64 {
            b64s[k + shf_div64] = a64s_hi[k - 1] + a64s_lo[k] * p_hi;
        }
        self.a64s
            .iter()
            .zip(a64s.iter())
