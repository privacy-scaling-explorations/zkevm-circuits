...
impl<F: Field> ExecutionConfig<F> {
    ...
    pub(crate) fn configure(
        ...
        meta.create_gate("q_step", |meta| {
            ...
            cb.condition(q_step.clone(), |cb| {
                cb.require_boolean(
                    "step.step.is_create is boolean",
                    step_curr.state.is_create.expr(),
                );
                cb.require_boolean(
                    "step.step.is_root is boolean",
                    step_curr.state.is_root.expr(),
                );
            });
    ...
    fn assign_same_exec_step_in_range(
        ...
        assert_eq!(height, 1);
        assert!(step.step.rw_indices.is_empty());
    ...
    fn assign_exec_step(
        ...
        // Make the region large enough for the current step and the next step.step.
        ...
        // Also set the witness of the next step.step.
    ...
    fn check_rw_lookup(
        ...
        // Circuit is in the set of rw operations generated by the step.step.
        ...
        if step.step.rw_indices.len()
            != assigned_rw_values.len() + step.step.copy_rw_counter_delta as usize
        {
            log::error!(
                "step.step.rw_indices.len: {} != assigned_rw_values.len: {} + step.step.copy_rw_counter_delta: {} in step: {:?}", 
                step.step.rw_indices.len(),
        ...
        for (idx, assigned_rw_value) in assigned_rw_values.iter().enumerate() {
            ...
            // generated at the end of the step.step.
            let idx = if is_rev {
                step.step.rw_indices.len() - rev_count
            } else {
                idx - rev_count
            };
            let rw_idx = step.step.rw_indices[idx];
