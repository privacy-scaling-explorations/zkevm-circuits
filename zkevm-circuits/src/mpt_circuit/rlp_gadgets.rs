...
impl<F: Field> RLPListGadget<F> {
    pub(crate) fn construct(cb: &mut ConstraintBuilder<F, Table>, bytes: &[Expression<F>]) -> Self {
...
impl<F: Field> RLPListDataGadget<F> {
    pub(crate) fn construct(cb: &mut ConstraintBuilder<F, Table>) -> Self {
