...
pub(crate) fn compress(input: &[u8]) -> Vec<u8> {
    ...
    for byte in input {
        ...
        let mut len = HUFFMAN_CODE_LEN[*byte as usize];
        while len > 0 {
            match len.cmp(&word_remaining) {
                std::cmp::Ordering::Less => {
                    word_remaining -= len;
                    word |= (code << word_remaining) as u8;
                    len = 0;
                }
...
mod tests {
    ...
    fn test_compress() {
        ...
        let word1 = (0x1ff8 >> 5) as u8;
        let word2 = ((0x1ff8 << 3) | (0x7fffd8 >> 20)) as u8;
        let word3 = (0x7fffd8 >> 12) as u8;
        let word4 = (0x7fffd8 >> 4) as u8;
        let word5 = (0x7fffd8 << 4) as u8;

        assert_eq!(result, vec![word1, word2, word3, word4, word5]);
    }
}
