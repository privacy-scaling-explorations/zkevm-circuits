...
//         is_write: false,
//         account_address:
// address!("0x000000000000000000000000000000000cafe002"),         field_tag:
// AccountFieldTag::CodeHash,         value: U256::zero(),
//         value_prev: U256::zero(),
//     }];
//     let overrides = HashMap::from([((AdviceColumn::AddressLimb0, 0),
// Fr::zero())]);
...
//         is_write: false,
//         account_address:
// address!("0x0000000000000000000000000000000000010000"),         field_tag:
// AccountFieldTag::CodeHash,         value: U256::zero(),
//         value_prev: U256::zero(),
//     }];
...
//     }];
//     let overrides = HashMap::from([((AdviceColumn::StorageKeyByte1, 0),
// Fr::one())]);
...
//     // This will trigger two errors: an RLC encoding error and the "fit into
// u8", we     // remove the first one
...
//         value: U256::zero(),
//     }];
//     let overrides = HashMap::from([((AdviceColumn::IsWrite, 0),
// Fr::from(4))]);
...
//     assert_error_matches(verify(vec![second, first]), "limb_difference fits
// into u16"); }
...
//         is_write: true,
//         account_address:
// address!("0x1000000000000000000000000000000000000000"),         field_tag:
// AccountFieldTag::CodeHash,         value: U256::zero(),
//         value_prev: U256::zero(),
//     };
...
//         is_write: true,
//         account_address:
// address!("0x2000000000000000000000000000000000000000"),         field_tag:
// AccountFieldTag::CodeHash,         value: U256::one(),
//         value_prev: U256::one(),
//     };
...
// field_tag: AccountFieldTag::Nonce,             value: Word::zero(),
//             value_prev: Word::zero(),
//         },
//     ];

//     // overriding first_different_limb to be in AddressLimb0 instead of Tag.
// The     // limb difference between the two rows here is still 1, so no
// additional     // overrides are needed.
//     let overrides = HashMap::from([
...
//     assert_error_matches(verify(rows), "non-first access reads don't change
// value"); }
...
//     let result = prover(vec![], overrides).verify_at_rows(N_ROWS - 1..N_ROWS,
// N_ROWS - 1..N_ROWS);     assert_error_matches(result, "rw_counter increases
// by 1 for every non-first row"); }
...
//     assert_error_matches(verify(rows), "first access to new stack address is
// a write"); }
...
//     assert_error_matches(
//         verify(rows),
//         "if previous row is also Stack with unchanged call id, address change
// is 0 or 1",     );
// }
...
//     let tags: BTreeSet<usize> = RwTableTag::iter().map(|x| x as
// usize).collect();     for i in 0..16 {
//         if tags.contains(&i) {
//             continue;
//         }
//         let bits: [Fr; 4] = i
...
//     let overrides = HashMap::from([((AdviceColumn::InitialValue, 0),
// Fr::from(10))]);
...
//         tx_id: 1,
//         account_address:
// address!("0x0000000000000000000000000000000004356002"),         is_warm:
// true,         is_warm_prev: false,
//     }];
...
// fn prover(rows: Vec<Rw>, overrides: HashMap<(AdviceColumn, isize), Fr>) ->
// MockProver<Fr> {     let updates = MptUpdates::mock_from(&rows);
//     let circuit = StateCircuit::<Fr> {
//         rows,
//         updates,
//         overrides,
...
//         .verify_at_rows(N_ROWS - used_rows..N_ROWS, N_ROWS -
// used_rows..N_ROWS) }
...
//             // fields of halo2_proofs::dev::metadata::Constraint aren't
// public, so we have             // to match off of its format string.
