...
impl<F: FieldExt> IsEqualInstruction<F> for IsEqualChip<F> {
    fn assign(
        &self,
        region: &mut Region<'_, F>,
        offset: usize,
        ...
        let value = lhs.zip(rhs).map(|(a, b)| a - b);

        self.config.is_zero_chip.assign(region, offset, value)?;
...
mod tests {
    ...
    impl<F: FieldExt, const RHS: u64> Circuit<F> for TestCircuit<F, RHS> {
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    for (idx, (value, check)) in self.values.iter().cloned().zip(checks).enumerate()
                    {
                        ...
                        chip.assign(&mut region, idx + 1, Some(value.into()), Some(RHS.into()))?;
                    }
