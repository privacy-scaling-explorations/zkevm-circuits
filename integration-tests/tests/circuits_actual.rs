    ...
    ($name:ident, $block_tag:expr) => {
        paste! {
            ...
            async fn [<serial_test_evm_ $name>]() {
                ...
                test_evm_circuit_block(*block_num, true).await;
            }
            ...
            async fn [<serial_test_state_ $name>]() {
                log_init();
                let block_num = GEN_DATA.blocks.get($block_tag).unwrap();
                test_state_circuit_block(*block_num, true).await;
            }
            ...
            async fn [<serial_test_tx_ $name>]() {
                log_init();
                let block_num = GEN_DATA.blocks.get($block_tag).unwrap();
                test_tx_circuit_block(*block_num, true).await;
            }
            ...
            async fn [<serial_test_bytecode_ $name>]() {
                log_init();
                let block_num = GEN_DATA.blocks.get($block_tag).unwrap();
                test_bytecode_circuit_block(*block_num, true).await;
            }
