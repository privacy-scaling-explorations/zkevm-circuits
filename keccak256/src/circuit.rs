    ...
    pub fn configure(
        ...
        meta.create_gate("running sum", |meta| {
            ...
            let expr_next_13_acc =
                (acc_13 - acc_13_next.clone()) - coef_13.clone() * slice_13;
            let expr_next_9_acc =
                (acc_9 - acc_9_next) - coef_9.clone() * slice_9;
            let step2_acc =
                meta.query_advice(block_count_cols[1], Rotation::cur());
            let step2_acc_next =
                meta.query_advice(block_count_cols[1], Rotation::next());
            ...
            let mut checks = vec![
                ...
                q_enable.clone() * is_final * acc_13_next.clone(),
            ];
            // block_count acc correctness
            if step == 1 {
                checks.push(q_enable.clone() * block_count.clone());
                checks.push(q_enable.clone() * (step2_acc_next - step2_acc));
                checks.push(q_enable.clone() * (step3_acc_next - step3_acc));
            } else if step == 2 {
                checks.push(
                    q_enable.clone()
                ...
                checks.push(
                    q_enable.clone()
                        * (step3_acc_next - step3_acc - block_count),
                );
            } else {
                unreachable!("step < 4");
            }
